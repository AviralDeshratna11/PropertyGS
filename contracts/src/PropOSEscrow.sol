...
contract PropOSEscrow is ReentrancyGuard {
    ...
    event TitleVerified(uint256 indexed dealId, string titleDeedHash, address payoutAddress);
     ...
     */
    function verifyTitle(uint256 dealId, string calldata _titleDeedHash, address _payoutAddress)
        external
        onlyParticipant(dealId)
    {
        ...
        d.payoutAddress = _payoutAddress;
        d.state = EscrowState.TitleVerified;

        emit TitleVerified(dealId, _titleDeedHash, _payoutAddress);
    ...
    function _releaseFunds(uint256 dealId) internal nonReentrant {
        ...
        // Release 95% to payoutAddress if set, otherwise to seller (DLD Direct Payment mandate prefers title payee)
        address beneficiary = d.payoutAddress == address(0) ? d.seller : d.payoutAddress;
        (bool sent, ) = payable(beneficiary).call{value: d.releaseAmount}("");
    ...
    function getDealStatus(uint256 dealId) external view returns (
        ...
        Deal storage d = deals[dealId];
        return (
            d.state, d.payoutAddress, d.totalAmount, d.releaseAmount, d.retentionAmount,
